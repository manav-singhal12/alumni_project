...
import { useJobPostingMutation, useAllJobsQuery } from "../../redux/api/jobs.ApiSlice.js";
...
function GetAllJobs() {
  const { data: jobs, error, isLoading: jobsLoading } = useAllJobsQuery();
  console.log("the jobs are ", jobs);

  const [postJob, { isLoading: postLoading }] = useJobPostingMutation();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Optionally, reset form fields after a successful post
      setTitle("");
      setCompany("");
      setLocation("");
      setJobType("");
  ...
  return (
    <main className="min-h-screen p-6 bg-[#e0f2f1]">
      <div className="container mx-auto">
        ...
        {userInfo ? (
          userInfo.data.user.role === "alumni" ? (
            <section
              id="post-job"
              className="mb-10 bg-white p-6 rounded-lg shadow-md text-center mx-4"
            >
              {!showForm ? (
                <button
                  onClick={handleToggleForm}
                  className="w-full md:w-3/4 lg:w-1/2 h-16 px-6 py-3 bg-[#004d40] text-white font-semibold rounded hover:bg-[#00796b] transition duration-300"
                >
                  Post a Job
                </button>
              ) : (
                <>
                  ...
                  >
                    ...
                    {(postLoading || jobsLoading) && <Loader />}
                  </form>
                </>
              )}
            </section>
        ...
        <section id="job-listings" className="mb-10 px-4">
